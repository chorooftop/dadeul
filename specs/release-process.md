...
- **콜드스타트 실측치 (2026-08-22, 리비전 `dadeul-api-00002-q9b`)**
  ...
  회당 17분 이상 방치하고 **5회** 측정했다.

  | 구간 | 값 |
  |---|---|
  | 클라이언트 TTFB (한국 → 도쿄) | 2.221 / 1.992 / 1.948 / 2.134 / 2.236 s |
  | └ 평균 / 중앙값 | **2.106s / 2.134s** |
  | └ min~max (편차 0.288s) | 1.948 ~ 2.236s |
  | 서버측 지연 (`httpRequest.latency`) | 1.43 ~ 1.65s |
  | 요청 도착 → 앱 `listening` | 약 1.74s |
  | └ 앱 자체 부팅 (로컬 실측) | **0.26s** |
  | └ 컨테이너 기동·이미지 마운트 | 약 1.4s |
  | 웜 요청 (참고) | 서버측 2~84ms |

  편차가 0.29s로 좁아 **2.1초가 안정적인 실측치**다. 나머지 0.5~0.6s는 한국↔도쿄
  네트워크·TLS 왕복이라 서버를 어떻게 고쳐도 남는다.
