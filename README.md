...
다들(dadeul) 서비스의 **TOM(Topological Object Manifest)** 명세 저장소.
네이티브 앱(iOS Swift / Android Kotlin)과 Node API가 공유하는 단일 명세 소스다.
...
- iOS app: `~/Desktop/dadeul/dadeul-ios` _(not created yet)_
- Android app: `~/Desktop/dadeul/dadeul-android` _(not created yet)_
- Node API: `~/Desktop/dadeul/dadeul-api` _(not created yet)_
...
└── tools/tom/              # TOM CLI (플레이북 기반 자체 구현)
    ├── bin/tom             # POSIX sh 런처
    ├── lib/*.mjs           # 구현 (의존성 0, node >= 18.17)
...
- [x] `tom init` — MANIFEST.md 생성 (다들 도메인 기준 포함)
- [x] 씨앗 어휘 — 제품 정책 인터뷰 기반 (코드 없음, 명세가 유일 진실)
- [x] 첫 spec — `spec-weather-vote-widget` (draft)
- [x] CLAUDE.md / AGENTS.md + hooks
- [ ] 코드 레포 생성 후 구현 앵커 연결
